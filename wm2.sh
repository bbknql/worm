...
if [ -n "$worm" ];then
        ...
        if [ $cts -eq $vl ];then
                ...
                if [[ $container =~ "Up" ]];then
                        while true
                        do
                                key=$(docker exec -it wormholes /usr/bin/ls -l /wm2/.wormholes/wormholes/nodekey)
                                if [ -n "$key" ];then
                ...
                if [ $cts -lt $vt5 ];then
                        if [ -f /wm2/.wormholes/wormholes/nodekey ];then
                                echo "Clearing historical data ............"
                                cp /wm2/.wormholes/wormholes/nodekey /wm2/nodekey
                                rm -rf /wm2/.wormholes
                                mkdir -p /wm2/.wormholes/wormholes
                                mv /wm2/nodekey /wm2/.wormholes/wormholes/
                        else
                                read -p "Enter your private key：" ky
                        fi
...
if [ -n "$ky" ]; then
        mkdir -p /wm2/.wormholes/wormholes
        if [ ${#ky} -eq 64 ];then
                echo $ky > /wm2/.wormholes/wormholes/nodekey
        elif [ ${#ky} -eq 66 ] && ([ ${ky:0:2} == "0x" ] || [ ${ky:0:2} == "0X" ]);then
                echo ${ky:2:64} > /wm2/.wormholes/wormholes/nodekey
        else
...
docker run -id -p 30303:30303 -p 8545:8545 -v /wm2/.wormholes:/wm2/.wormholes --name wormholes wormholestech/wormholes:v1 >/dev/null 2>&1 &

while true
do
        s=$(docker ps -a|grep "Up"|awk '{if($NF == "wormholes") print $NF}'|wc -l)
        key=$(docker exec -it wormholes /usr/bin/ls -l /wm2/.wormholes/wormholes/nodekey 2>/dev/null)
        if [[ $s -gt 0 ]] && [[ "$key" =~ "nodekey" ]];then
                echo "Your private key is:"
                docker exec -it wormholes /usr/bin/cat /wm2/.wormholes/wormholes/nodekey
                echo -ne "\n"
